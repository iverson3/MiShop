<template>
	<view>
		<divider></divider>
		<card headTitle="发票类型" :bodyPadding="true" cardStyle="background-color: #FFFFFF;">
			<mi-radio-group :label="label1" :selected.sync='label1.selected'></mi-radio-group>
			<view class="text-light-muted line-h-md">
				电子发票与纸质发票具有相同的法律效力，可作为报销、售后、维权凭证，推荐使用电子发票，不怕丢失，更方便、环保。
			</view>
		</card>
		<divider></divider>
		<card headTitle="发票抬头" :bodyPadding="true" cardStyle="background-color: #FFFFFF;">
			<mi-radio-group :label="label2" :selected.sync='label2.selected'></mi-radio-group>
		</card>
		<divider></divider>
		
		<view class="p-2 border-bottom d-flex a-center bg-white">
			<text class="font-md">个人姓名：</text>
			<text class="font-md ml-2">{{ invoiceName }}</text>
		</view>
		<view class="p-2 border-bottom d-flex a-center bg-white">
		...
		<view class="p-2 border-bottom d-flex a-center bg-white">
			...
			<text class="font-md ml-2">13396095889</text>
		</view>
...
<script>
	...
	export default {
		...
		computed: {
			invoiceName() {
				return this.label2.list[this.label2.selected].name
			}
		},
		methods: {
